...
<div>
                            ...
                            <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                <div class="card-header align-items-center  border-bottom-dark px-0">
                                    ...
                                    <div class="icons d-flex">
                                        <button class="btn ml-2 p-0 kt_notes_panel_toggle" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" v-if="$parent.permissions.includes('payment-methods-manage')">
                                ...
                                <div class="card-body">
                                    <div>
                                        <div class=" table-responsive" id="printableTable">
                                            ...
                                            <select name="productUnitTable_length" aria-controls="productUnitTable" class="" v-model="limit" v-on:change="fetchPaymentMethods()">
                                            ...
                                            <div id="productUnitTable_filter" class="dataTables_filter"><label>Search:<input type="search" class="" placeholder="" aria-controls="productUnitTable" v-model="searchParameter" @keyup="fetchPaymentMethods()"></label></div>
                                                <table id="productUnitTable" class="display dataTable no-footer" role="grid">
                                                    <thead class="text-body">
                                                        <tr role="row">
                                                            ...
                                                            <th class="sorting" tabindex="0" aria-controls="productUnitTable" rowspan="1" colspan="1" aria-label="paymentMethod: activate to sort column ascending" style="width: 95.5288px;" @click="sorting('name')" :class="(this.$data.sortType == 'asc' || this.$data.sortType == 'ASC') && this.$data.sortBy == 'name'  ? 'sorting_asc' : (this.$data.sortType == 'desc' || this.$data.sortType == 'DESC') && this.$data.sortBy == 'name' ? 'sorting_desc' : 'sorting'">
                                                                Name
                                                            </th>
                                                            ...
                                                            <th  v-if="$parent.permissions.includes('payment-methods-manage')" class="no-sort sorting_disabled" rowspan="1" colspan="1" aria-label="Action" style="width: 53.1891px;">
                                                                Action
                                                            </th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="kt-table-tbody text-dark">
                                                        <tr class="kt-table-row kt-table-row-level-0 odd" role="row" v-for="paymentMethod in paymentMethods" v-bind:key="paymentMethod.payment_method_id">
                                                            <td class="sorting_1">
                                                            ...
                                                            <td>
                                                                 {{ paymentMethod.payment_method_environment == '1' ? 'Live' : 'Sandbox' }}
                                                            </td>
                                                            <td>
                                                               {{ paymentMethod.payment_method_status == '1' ? 'Active' : 'Inactive' }}
                                                            </td>
                                                            <td  v-if="$parent.permissions.includes('payment-methods-manage')">
                                                            <a href="javascript:void(0)" class=" click-edit" id="click-edit1" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" @click="editPaymentMethod(paymentMethod)"><i class="fa fa-edit"></i></a>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                                <ul class="pagination pagination-sm m-0 float-right">
                                                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]"><button class="page-link" href="#" @click="fetchPaymentMethods(pagination.prev_page_url)">Previous</button></li>

                                                    <li class="disabled"><button class="page-link text-dark" href="#">Page {{ pagination.current_page }} of {{ pagination.last_page }}</button></li>

                                                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item"><a class="page-link" href="#" @click="fetchPaymentMethods(pagination.next_page_url)">Next</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="offcanvas offcanvas-right kt-color-panel p-5 kt_notes_panel" v-if="display_form" :class="display_form ? 'offcanvas-on' : ''">
        <div class="offcanvas-header d-flex align-items-center justify-content-between pb-3">
            <h4 class="font-size-h4 font-weight-bold m-0">Add paymentMethod</h4>
        ...
        <form id="myform">
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <small class="form-text text-danger" v-if="errors.has('environment')" v-text="errors.get('environment')"></small>
                    </div>
                    <div class="form-group">
                        <label class="text-dark">Status </label>
                        <select v-model="paymentMethod.status">
            ...
            <button type="button" @click="addUpdatepaymentMethod()" class="btn btn-primary">Submit</button>
        </form>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        addUpdatepaymentMethod() {
            this.$parent.loading = true;
            var url = '/api/admin/payment_method/' + this.paymentMethod.id;
