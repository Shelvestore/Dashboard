...
<!--    {{'sections.home-'.$template['template_postfix'].'-section'}}-->
    ...
    <script>
        ...
        function BrandSlider() {
            $.ajax({
                type: 'get',
                url: "{{ url('') }}" +
                    '/api/client/brand?getDetail=1&page=1&limit=10&getGallary=1&language_id=' + languageId +
                    '&sortBy=id&sortType=ASC',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    clientid: "{{ isset(getSetting()['client_id']) ? getSetting()['client_id'] : '' }}",
                ...
                success: function(data) {
                    if (data.status == 'Success') {
                        $(".brand-slider-show").html('');
                        const templ = document.getElementById("brand-slider-template");
                        // clone.querySelector(".single-text-chat-li").classList.add("bg-blue-100");
                        for (i = 0; i < data.data.length; i++) {
                            const clone = templ.content.cloneNode(true);
                            // clone.querySelector(".single-text-chat-li").classList.add("bg-blue-100");
                            // clone.querySelector(".brand-slider-url").setAttribute('href', '/shop?category=' +
                            //     data.data[i].id);
                             clone.querySelector(".brand-slider-image").setAttribute('src', '/gallary/' + data
                         .data[i].gallary);
                          //  clone.querySelector(".brand-slider-title").innerHTML = data.data[i].brand_name;
                            $(".brand-slider-show").append(clone);
                        }
                        getSliderSettings("brand-slider-show");
                    }
                },
                error: function(data) {},
            });
